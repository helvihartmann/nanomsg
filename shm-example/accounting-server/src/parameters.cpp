    ...
    process(process_)
{
    ...
    while ((opt = getopt_long (argc, argv, "hi:n:t:u:", longopts, NULL)) != -1)
        switch (opt)
    {
        case 'h':
            ...
            std::cout << "--timeout     -t      timeout for agents before talking to server" << std::endl;
        ...
        case 't':
            timeout = atoi(optarg);
            if ((timeout < 0)) {
                printf("ERROR -t: please enter vaild timeout for agents\n");
                exit(1);
            }
            break;
